...
## Workflow composition checkpoint

VideoAnalysisWorkflow now wraps the executor through the shared native entrypoint helper. The media-processor declares the class in development, staging and production; the jobs Worker references the corresponding media-processor script in all three. Binding invariants assert the exact names, class and script. VIDEO_ANALYSIS_ENABLED remains false everywhere. The enabled composition now supplies the real attempt, submission, reconciliation and stage-fact stores, a derived-artifact HEAD reader, and a source verifier that compares the immutable seal row against R2 ETag, object version, size and content type, plus the checksum when R2 exposes it. No source-body rehash occurs.

The focused binding, composition and queue suites passed with 22 tests and 85 assertions. The focused PostgreSQL suite passed with 21 tests and 217 assertions, including source replacement, size and content-type rejection. The native Workerd helper tests passed 2/2; the existing real-fetch status API tests passed 2/2 with no injected fetch. The first Workerd start failed because the jobs test pool's processor stub lacked the newly declared class; the stub now declares that binding target. That stub is not execution evidence. The helper tests exercise authority dispatch and fail-closed construction, not an end-to-end encode through the exported production class. bun run check exited 0 after correcting the step-interface type. Composed success and fault drills remain owed.

## Publication wakeup migration amendment

Before the wakeup amendment, main was rechecked at 6620174f416ca87a07c3386f81f6672ee9f8d643. Its inventory ends at 0120_hns_root_health_renewal_recovery.sql. The publication wakeup ledger is added to the existing unmerged video migration candidate, with no second video migration number. The previously recorded final-number repair remains deferred to the single PR-preparation rebase. Wakeup action identities are the actual moderation action id for approval and a separately named publication-retry action id for author retry; retries are not represented as moderator actions.

The wakeup ledger is now written in the same transaction as last-hold approval or publication-only retry. The latter also creates the new creation revision's ordinary launch intent. Scheduled delivery acknowledges only after sendEvent succeeds; lost responses replay the same action identity. Terminal or missing instances reset the existing intent to a capped continuation, and the queue remains the sole launcher. The sweep also admits a retained publication phase, covering termination after decision commit but before publication. A wakeup event does not authorize effects; the executor reloads PostgreSQL before publishing.

Validation: the focused publication and migration PostgreSQL suites exited 0 with 34 tests and 263 assertions. They include actual approval replay, present/terminal/missing delivery, lost notify response, and superseded-creation delivery. The focused launcher/queue/executor run exited 0 with 16 tests and 367 assertions; a subsequent executor run including approval before wait and while entering wait exited 0 with 8 tests and 353 assertions. bun run check exited 0. These do not yet constitute the final composed native-entrypoint encode drills.


## Composed execution checkpoint

The composed gate now drives makeMediaProcessorQueueWorker and the exported VideoAnalysisWorkflow class in Workerd against real PostgreSQL application and repository code. Its step primitive controls durable-result replay and fault boundaries; it does not kill a hosted Cloudflare instance. Qencode transport, source grants, provider responses and R2 are local fixtures. The Qencode adapter and artifact sealer execute normally. Recognition and safety use fixture responses at their provider ports, so this does not establish concrete ACR or moderation composition.

The gate exited 0 with 14 Workerd cases and its enclosing PostgreSQL test passed. The durable transcript is [composed-workflow-gate.txt](evidence/video-execution-2026-09-05/composed-workflow-gate.txt). It names the plain success case, drills 1, 3, 4 and 7, membership-loss fail-closed drill 5, database failure after accepted start, event-sequence changes during inspection, approval before and during the wait, continuations from allocated, started and accepted work, and sealed-audio recovery after a failed fact write and expired provider output. Success reaches one Post without a test-side publish call; publication replay also preserves one registration intent. Both changed-poster retry and a terminal provider job followed by technical retry create fresh attempts.

The composed tests exposed an invalid sealed-poster disposition: invalid JPEG bytes now become the existing typed poster rejection instead of repeated transport errors. Confirmed rejection during the reconciliation window resolves the attempt before exposing ordinary failure. A separate executor regression passed with the nine-test executor suite, exit 0, 360 assertions. The focused publication PostgreSQL, Qencode and executor run passed 51 tests with 629 assertions, exit 0, including the shared PostgreSQL fixture refactor.

The Workerd harness selects the published CommonJS pg-protocol and pg-cloudflare entrypoints to avoid the workers-sdk Vitest module-resolution issue 12984. The global setup creates and removes a uniquely named local test database, because concrete Hyperdrive composition intentionally pins the api_next schema. The standard PostgreSQL gate invokes this composed suite; no provider credentials are needed.

Drill 2 remains the ingestion seal/finalize recovery proof owned by api-video-execution-completion; this checkpoint does not claim a new pass for it. Drill 6 remains the browser restart and multipart recovery proof owned by solid-video-original-audio-runtime. Required-reconciliation operator tooling remains follow-up: it must observe the existing provider token and call the fenced resolution store without resubmission.

Staging still requires concrete providers, the grant table and gateway Worker, the read token in Infisical, the reason-code waiver, and the delivery lane. VIDEO_ANALYSIS_ENABLED remains false, and this checkpoint makes no deployment or live-provider claim. The single PR-preparation rebase, migration ordinal collision repair, and full PostgreSQL gate remain integration work.

Final local validation: bun run check exited 0. bun run test exited 0 with 2,994 Bun unit tests, 20 Node tests and 135 Workerd tests. The first ordinary run failed the exact production Workflow-name assertion because it omitted the new dormant video binding; the assertion now includes that binding while retaining the song and DATA names. The added reconciliation regression initially required formatting; after formatting, check passed. The final summaries and test names are preserved in composed-workflow-validation.json. Full PostgreSQL validation remains reserved for PR preparation.


## Integration preparation — 2026-09-05

The owner accepted the composed tranche through pre-rebase source 1cbb7594dad4975e75f654f53845c20b1f49312e. Control-plane commit 23ce5dd4f70029587eddee5d85e9e5780d76bea8 records publication and squash-merge authorization after the required gates. The single rebase targeted fetched origin/main 386be35a87163bd6b93ab49c41666a7442fd17e2. That inventory contains merged HNS migrations 0120 and 0121; the unmerged video candidate is now 0122_video_workflow_execution.sql. The baseline generator ran successfully after rebuilding checksums. The normalized schema and reset files were already identical to the regenerated merged result; neither merged HNS migration changed.

The unused runOriginalVideoAnalysis consumer-era interpreter, its unit suite and its FFmpeg publication-replay fixture have been removed. Shared analysis contracts, binding/caption hashing helpers and FFmpeg artifact tests remain. Video now has one production orchestration executor. Five newly unused fixture exports were made private, preserving main's stricter Knip baseline without increasing it.

Private terminal errors identify superseded authority, rejected domain analysis, invalid stage invariants and publication membership loss. The class runner maps those errors to Cloudflare NonRetryableError inside the durable callback, before the platform retries it. Database and transport exceptions retain ordinary step retries. The real Workerd helper's five tests pass, including terminal-versus-database classification. Membership loss still consumes the separate continuation cap and rests in required reconciliation without a Post. Its public reason and provider_submission_unconfirmed remain one combined waiver and client-release disposition; no wire code changes here.

The composed PostgreSQL gate is now explicitly classified as owning its own isolated database in the suite inventory introduced on main. Historical patch files in this evidence directory are checkpoint evidence, not executable instructions for the current control-plane record. The accepted checkpoint has been registered; use the live record for dispatch.

Integration validation: final baseline generation and bun run check exited 0. The ordinary test command exited 0 with 3,039 Bun, 20 Node and 141 Workerd tests. The full PostgreSQL command completed within its timeout: isolated 35/35 passed, general 364 passed and one failed because the foundation's explicit table inventory omitted media_video_publication_wakeups. After adding that table and aligning refusal diagnostics to ordinal 0122, the focused foundation/migration run exited 0 with all 22 tests and 225 assertions. No continuation split was needed. The standard full command exited 1 and is not described as a pass; the remote postgres17 gate remains required. Its composed Workerd child passed all 14 named cases. Durable per-test summaries and exact run exit statuses are in evidence/video-execution-2026-09-05/integration-validation.json.
