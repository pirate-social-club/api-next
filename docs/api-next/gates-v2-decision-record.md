...
boundary and registry, a fake-provider conformance fixture, dependency guards,
and the Postgres ledger. The evaluator, persistence services, routes, client
presentations, and real provider implementations are separate slices.
...
- `credential.subject_unique`: a stable credential subject without implied
  liveness;
- `document.valid`, `age.minimum`, `nationality.allowed`, and `gender.marker`;
- `asset.ownership`; and
...
linked to one authority and scope.
...
- proof sessions with actor, intent, request hash, issuer scope, protocol, and
  environment;
...
The pure engine returns a winning witness. Consumables commit only from that
witness after evaluation, so a failed sibling cannot burn a proof and AST order
...
Postgres, the same additions also exist as numbered delta `0008`; that delta is
deployment bookkeeping for api-next, not a data-transfer or compatibility
path. Fresh databases produced by the cumulative baseline and by ordered
deltas must have identical catalogs.

Provider additions must be adapter-local. The conformance fixture proves a fake
provider can register without editing the engine, evidence schema, routes, or
contract enums. Dependency-direction checks land with the adapter boundary,
before any real provider. If adding Self, ZKPassport, Very, World ID, Humanity,
or another provider requires a core edit, the abstraction has failed and must
be reviewed rather than expanded with another provider case.
