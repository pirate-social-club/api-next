...
/** The exact statements this store runs, exported so a baseline-backed test
 * can prepare them against the migrated schema and catch column drift. */
export const songLibraryStatements = {
  owner: `SELECT persona_id FROM personas JOIN users ON users.user_id = personas.account_id WHERE persona_id = $2 AND account_id = $1 AND personas.status = 'active' AND users.status = 'active'`,
  list: `WITH sessions AS (${sessions(false)}), history AS (
  ...
  trending: `WITH sessions AS (${sessions(true)}), ranking AS (
...
const failure = (cause: unknown) =>
  new InternalError({ message: "Song collection could not be loaded", cause });
    ...
    list: (input) =>
      Effect.gen(function* () {
        ...
        const owner = yield* db.execute<{ persona_id: string }>({
          ...
          text: songLibraryStatements.owner,
        ...
        const result = yield* db.execute({
          ...
          text: songLibraryStatements.list,
          values: [
            input.accountId,
            input.personaId,
        ...
        Effect.mapError((error) => (error instanceof NotFound ? error : failure(error))),
      ),
    trending: () =>
      Effect.gen(function* () {
        const db = yield* ControlPlaneDb;
        const result = yield* db.execute({
          ...
          text: songLibraryStatements.trending,
          values: [],
          readonly: true,
        });
