import type { Iso3166Alpha2 } from "@pirate/domain/verification";

// Self returns ISO-3166 alpha-3 nationality values. Keep this copy local to
// the provider boundary rather than widening the frozen domain exports.
...
const ISO2_BY_ISO3 = new Map<string, string>(
...
export function normalizeSelfCountry(value: string): Iso3166Alpha2 | undefined {
  ...
  if (/^[A-Z]{2}$/u.test(normalized) && ISO2_CODES.has(normalized)) {
