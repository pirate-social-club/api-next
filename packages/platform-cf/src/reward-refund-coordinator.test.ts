...
import {
  deriveRewardRefundEffectId,
  makeRewardRefundCoordinator,
} from "./reward-refund-coordinator.ts";
...
const candidate: RewardRefundCandidate = {
  fundingEffectId: "funding-901",
  legId: "leg-a",
  funderAccountId: "account-a",
  amountAtomic: 901n,
  destinationAddress: RECIPIENT,
  proRataNumeratorAtomic: 901n,
  proRataDenominatorAtomic: 901n,
...
function harness() {
  let progress: RewardRefundProgress | null = null;
  let sends = 0;
...
describe("reward refund coordinator", () => {
  test("returns one terminal contribution exactly once and replays the confirmed receipt", async () => {
    const state = harness();
    ...
    const replay = await Effect.runPromise(coordinator.refund(candidate.fundingEffectId));
