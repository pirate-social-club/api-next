...
export type CommunityCreationRepositoryOptions = Readonly<{
  /** Active communities one account may have created; test seam, defaults to the ratified cap. */
  readonly optional_route_account_community_cap?: number;
  ...
  return Effect.gen(function* () {
    const result = yield* transaction.execute<Row>({
      ...
      text: `SELECT revision.intent_id, revision.request_hash, revision.state_snapshot,
                    intent.status AS current_status, intent.draft AS current_draft,
                    EXISTS (SELECT 1 FROM nationality_requirement_states AS state
                             WHERE state.action_kind = 'community_creation'
                               AND state.intent_id = intent.intent_id
                               AND state.actor_id = intent.actor_id) AS retired_creator_nationality
    ...
    // Cutover expires obsolete nationality waits. Replay must return the
    // retired intent, never resurrect a removed verification action.
    const currentDraft = jsonValue(row.current_draft);
    const carriesNationality =
      currentDraft !== null &&
      typeof currentDraft === "object" &&
      "policy" in currentDraft &&
      policyCarriesNationalityRequirement(currentDraft.policy);
    if (
      row.current_status === "expired" &&
      (carriesNationality || row.retired_creator_nationality === true)
    ) {
      const current = yield* loadLockedIntent(
        transaction,
        input.actorId,
        String(row.intent_id),
        input.operation,
      );
      const document = current === null ? null : documentFromRow(current);
      if (document === null) return yield* Effect.fail(failure(input.operation, "invalid-row"));
      return document;
    }
...
export function makeControlPlaneCommunityCreationRepository(
  ...
  const configuredCap = options.optional_route_account_community_cap;
  const accountCommunityCap =
    configuredCap !== undefined && Number.isSafeInteger(configuredCap) && configuredCap >= 0
      ? configuredCap
      : OPTIONAL_ROUTE_ACCOUNT_COMMUNITY_CAP;
    ...
    Effect.gen(function* () {
      ...
      return yield* db.withTransaction((transaction) =>
        Effect.gen(function* () {
          ...
          const initialStatus = requirementFreeStatus(compiled);
      ...
      return yield* db.withTransaction((transaction) =>
        Effect.gen(function* () {
          ...
          if (
            ...
            (JSON.stringify(body.draft.persona) !== JSON.stringify(document.draft.persona) ||
              body.draft.public_name !== document.draft.public_name)
          ) {
            return yield* Effect.fail(failure("update", "constraint"));
          }
  ...
  /** Shared activation for commit-ready intents; `claim` carries the pre-amendment verified-subject quota slot. */
  const activateOptionalRouteV2 = (
    transaction: ControlPlaneTransaction,
    input: Parameters<CommunityCreationStoreService["commit"]>[0],
    body: Parameters<CommunityCreationStoreService["commit"]>[0]["body"],
    document: CommunityCreationIntentV2,
    providerId: string | null,
    compilation: SupportedCommunityGateCompilation,
    claim: Readonly<{
    ...
    Effect.gen(function* () {
      ...
      if (
        ...
        (claim !== null &&
          claim.evidence.receiptExpiresAt !== null &&
          Date.parse(claim.evidence.receiptExpiresAt) <= Date.parse(activationNow)) ||
        (claim !== null &&
          claim.evidence.assertionExpiresAt !== null &&
          Date.parse(claim.evidence.assertionExpiresAt) <= Date.parse(activationNow)) ||
        (claim !== null &&
          claim.approvalExpiresAt !== null &&
      ...
      if (document.draft.persona.kind === "create_new") {
        if (document.draft.public_name === undefined) {
          return yield* new CommunityOwnerSetupError(
            "Enter a public name before continuing community setup",
          );
        }
        const reserved = yield* reserveCommunityOwner(transaction, {
          accountId: input.actor.userId,
          intentId: input.intentId,
          publicName: document.draft.public_name,
        });
      ...
      const owner = yield* transaction.execute<Row>({
        ...
        text: `SELECT public_persona_projection(persona.persona_id) AS presentation
                 FROM personas AS persona JOIN persona_profiles AS profile USING (persona_id)
                WHERE persona.account_id=$1 AND persona.persona_id=$2 AND persona.status='active'
                  AND btrim(profile.display_name) <> ''
                  AND NOT EXISTS (SELECT 1 FROM persona_community_bindings WHERE persona_id=$2)`,
        values: [input.actor.userId, creatorPersonaId],
        readonly: true,
      });
      const publicOwner = Schema.decodeUnknownOption(PublicPersonaV1)(owner.rows[0]?.presentation);
      if (Option.isNone(publicOwner)) {
        return yield* new CommunityOwnerSetupError(
          "Choose an active, named profile that has not joined another community",
        );
      }
      const creatorPresentation: CommunityPersonaRolePresentationV1 = {
        role: "owner",
      ...
      if (claim !== null && subjectClaimId !== null) {
        ...
        yield* transaction.execute({
          ...
          values: [
            subjectClaimId,
            claim.evidence.subjectKeyId,
            input.actor.userId,
            claim.slotNumber,
            claim.approvalId,
            input.intentId,
            communityId,
            claim.evidence.proofSessionId,
            claim.evidence.evidenceReceiptId,
            claim.requirementHash,
          ],
          readonly: false,
        });
      }
    ...
    Effect.gen(function* () {
      ...
      if (creatorRequirement === undefined) {
        ...
        const createdResult = yield* transaction.execute<Row>({
          label: "community.creation.commit-v2.count-account-communities",
          text: `SELECT COUNT(*)::integer AS created
                   FROM communities
                  WHERE created_by_user_id = $1 AND status = 'active'`,
          values: [input.actor.userId],
          readonly: false,
        });
        const created = asNonNegativeInteger(oneRow(createdResult.rows)?.created);
        if (created === null) return yield* Effect.fail(failure("commit", "invalid-row"));
