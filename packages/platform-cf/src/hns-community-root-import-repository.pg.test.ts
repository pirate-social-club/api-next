...
/**
 * Every case builds a schema and installs the whole baseline, which is well
 * past bun's five-second default and grew again when the lifecycle migrations
 * landed. The budget is explicit so the suite fails on a real hang.
 */
const SCHEMA_BUDGET_MS = 180_000;
...
suite("community HNS root-import repositories", () => {
  test(
    "reports when a name is already attached to another community",
    async () => {
  ...
  test(
    "refuses provisional import of the retained operator root without admitting work",
    async () => {
      await withSchema(async (connection, admin) => {
        ...
        FROM operator_managed_root_registry_current`)
          ...
          {
            registry_reference: registryRef,
            registry_version: "1",
  ...
  test(
    "persists preparation, provider session, root-import session, and exact replay",
    async () => {
      await withSchema(async (connection, admin) => {
        expect(
          (
        ...
        const current = (actor = actorId, community = communityId) =>
          Effect.runPromise(
            communityStore.getCurrent({ actor_id: actor, community_id: community }),
          );
        ...
        try {
          ...
          expect(
            (
              await admin.query(
                "SELECT count(*)::integer AS count FROM hns_authority_provision_jobs",
              )
            ).rows,
          ).toEqual([{ count: 0 }]);
        } finally {
        ...
        const ownership = {
          ...
          challenge: {
            ...
            record: {
              type: "TXT" as const,
              txt: ["pirate-verification=community-import"] as const,
            },
            expires_at: expiresAt,
          },
          replayed: false,
        ...
        const claim = await admin.query(
          "SELECT * FROM claim_hns_authority_provision_job_v1($1,60)",
        ...
        expect(await held()).toBe(true);
        expect(
          (await finalizeCleanup(reclaimed.lease_fence, "failed", "session_expired")).rows[0]
            .outcome,
        ).toBe("failed");
  ...
  test(
    "serializes quota races, preserves replay, and retains abandoned admissions",
    async () => {
      await withSchema(async (connection, admin) => {
        const layer = makeDirectPostgresControlPlaneLayer(connection);
        const store = makeControlPlaneHnsCommunityRootImportRepository({
