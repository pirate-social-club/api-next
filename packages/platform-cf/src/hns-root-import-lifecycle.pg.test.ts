...
suite("HNS root-import lifecycle atomic PostgreSQL semantics (T08)", () => {
  test(
    "commits state, history, and requested jobs in one transaction",
    async () => {
  ...
  test(
    "rejects a stale expected revision as a serialization conflict",
    async () => {
      const schema = `hns_lifecycle_t08_${randomUUID().replaceAll("-", "")}`;
      const admin = new Client({ connectionString });
      await admin.connect();
  ...
  test(
    "replays an applied event identity without changing state or duplicating history",
    async () => {
      const schema = `hns_lifecycle_t08_${randomUUID().replaceAll("-", "")}`;
      const admin = new Client({ connectionString });
      await admin.connect();
  ...
  test(
    "refuses a transition outside the pure table and keeps the anchor and terminal phase immutable",
    async () => {
      const schema = `hns_lifecycle_t08_${randomUUID().replaceAll("-", "")}`;
      const admin = new Client({ connectionString });
      await admin.connect();
  ...
  test(
    "claims jobs in due order across roots and classes and fences finalize against lost leases",
    async () => {
      const schema = `hns_lifecycle_t08_${randomUUID().replaceAll("-", "")}`;
      const admin = new Client({ connectionString });
      await admin.connect();
  ...
  test(
    "retains the last useful error and budget exhaustion reason on provider failures",
    async () => {
      const schema = `hns_lifecycle_t08_${randomUUID().replaceAll("-", "")}`;
      const admin = new Client({ connectionString });
      await admin.connect();
