...
export function makeControlPlaneMediaSubmissionRepository(
  ...
  const transitionSimple = (
    ...
    update: (
      ...
      readonly referenceWakeup?: OutboxWrite;
    },
  ) =>
    Effect.gen(function* () {
      ...
      return yield* db.withTransaction((tx) =>
        Effect.gen(function* () {
          ...
          if (
            projection.event === "reference_bound" &&
            current.workflowRevision > 0 &&
          ...
          if (projection.referenceWakeup !== undefined) {
            if (
              projection.referenceWakeup.payload.kind !== "decision_wakeup" ||
              projection.referenceWakeup.payload.trigger !== "reference" ||
              !validId(projection.referenceWakeup.outboxEventId) ||
              !validId(projection.referenceWakeup.effectIdentity)
            )
              return yield* Effect.fail(fail(operation, "invalid-input"));
