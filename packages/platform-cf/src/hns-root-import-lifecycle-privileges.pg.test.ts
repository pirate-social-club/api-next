...
suite("HNS root-import lifecycle runtime privileges on PostgreSQL 17 (T14)", () => {
  test(
    "actual roles separately execute claim, transition, and finalization with bounded grant failures",
    async () => {
      ...
      try {
        ...
        await admin.query(
          `GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA ${schema} TO ${executor}`,
        );
        await admin.query(
          `GRANT INSERT, UPDATE, SELECT ON hns_root_import_lifecycle TO ${executor}`,
        );
        await admin.query(
          `GRANT INSERT, SELECT ON hns_root_import_lifecycle_history TO ${executor}`,
        );
        await admin.query(
