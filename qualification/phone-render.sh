...
# Usage: phone-render.sh <media-submission-id> preflight|execute
SUBMISSION=${1:?submission id}
MODE=${2:-}
[[ $SUBMISSION =~ ^media-submission-[0-9a-f-]{36}$ ]] || { echo "STOP: bad submission id" >&2; exit 2; }
SHORT=${SUBMISSION#media-submission-}; SHORT=${SHORT%%-*}
...
PRIVATE=$(mktemp -d /tmp/video-phone-render.XXXXXXXX)
chmod 700 "$PRIVATE"
...
read -r PLAN ATTEMPT SOURCE_KEY OUTPUT_KEY SONG_KEY < <(python3 - "$EVIDENCE/render-preflight.json" "$SUBMISSION" <<'PY'
import json,sys
...
if not r['song_post_id'].startswith(prefix_song) or not 0 < int(r['clip_duration_samples']) <= 720000:
    raise SystemExit('STOP: frozen song interval is not a 15-second-or-shorter song clip')
plan=f'song-video-plan:{submission}'
source=r['source_ref']
prefix=f'immutable/song-video-masters/{plan}/'
if r['plan_id']!=plan or not source.startswith('media://immutable/') or not r['dispatch_output_key'].removeprefix('media://').startswith(prefix):
    raise SystemExit('STOP: source or output binding differs')
song_key='immutable/media-operation-'+r['song_post_id'].removeprefix(prefix_song)+'/audio/1'
print(plan,r['attempt_id'],source.removeprefix('media://'),r['dispatch_output_key'].removeprefix('media://'),song_key)
PY
)
...
if [ "$MODE" != execute ]; then
  echo "usage: phone-render.sh <submission> preflight|execute" >&2
  exit 2
fi
...
bun qualification/branch-helpers.ts host-env render host.url input output \
  ...
  "SONG_VIDEO_RENDER_HOST_ID=phone-e2e-$SHORT" \
