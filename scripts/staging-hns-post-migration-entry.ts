 ...
 * ledger contains the reviewed cutover endpoint and exactly matches the
 * repository's pinned migration chain with matching checksums. Later reviewed
 * migrations are allowed, and only then does it verify the runtime and operator identities,
...
/**
 * Refuses unless the applied ledger contains the reviewed endpoint and exactly
 * matches the repository's pinned chain. Later pinned migrations are valid:
 * the coordinated release advances the schema after the HNS cutover. A
 * missing row, an unpinned row and a checksum mismatch are each named. This is
 * the guard that keeps a failed reset replay from reaching post-migration work.
 */
