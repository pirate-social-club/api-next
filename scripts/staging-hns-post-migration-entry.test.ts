...
describe("HNS staging post-migration entry point", () => {
  ...
  test("accepts reviewed migrations after the endpoint and refuses an unpinned row", async () => {
    const reviewedAfterEndpoint = {
      version: "0173_reviewed.sql",
      checksum: "9".repeat(64),
    };
    const reviewed = makePorts({
      readPinnedMigrations: async () => [
        ...pinnedMigrations.map((migration) => ({ ...migration })),
        reviewedAfterEndpoint,
      ],
      readMigrationLedger: async () => [
        ...pinnedMigrations.map((migration) => ({ ...migration })),
        reviewedAfterEndpoint,
    ...
    await expect(runHnsStagingPostMigration(makeInput(reviewed.ports))).resolves.toMatchObject({
    ...
    const missingReviewedMigration = makePorts({
      readPinnedMigrations: async () => [
        ...pinnedMigrations.map((migration) => ({ ...migration })),
        { version: "0173_unreviewed.sql", checksum: "9".repeat(64) },
      ],
    ...
    expect(
      await refusalOf(() => runHnsStagingPostMigration(makeInput(missingReviewedMigration.ports))),
    ).toMatchObject({
      step: "target_and_ledger",
      reason: "migration_missing",
      detail: { version: "0173_unreviewed.sql" },
