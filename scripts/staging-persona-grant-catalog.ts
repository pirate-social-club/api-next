...
export async function restoreReviewedResetGrants(
  ...
  const plan = reconcileResetGrants({ before, replay: replay.grants, reviewed });
  if (plan.unfulfilledReviewed.length) throw new Error("reset_reviewed_grants_unfulfilled");
  for (const grant of plan.reapply) {
  ...
  return { reapplied: plan.reapply.length, replay_created: plan.replayCreated.length };
}
