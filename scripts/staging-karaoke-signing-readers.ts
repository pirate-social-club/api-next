...
import {
  ...
  type loadKaraokeCollectorConfiguration,
...
/** Shared concrete readers for signing and pass recording. Authority is always
 * loaded from the retained journal and a fresh target-bound PostgreSQL read. */
export function makeStagingKaraokeSigningReaders(
  context: ReturnType<typeof loadKaraokeCollectorConfiguration>,
  apiToken: string,
): KaraokeSigningReaders {
  const { config, operator, assertion, journalTrust } = context;
