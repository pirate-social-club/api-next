...
export async function recordKaraokeCleanupPass(input: {
  ...
  // Cleanup is a post-fence-phase operation. Refuse a backward transition
  // before any provider mutation: once a target reached pre-reset, cleaning
  // again would mutate R2 before the verifier could reject the chain.
  for (const objectId of KARAOKE_RESET_OBJECT_IDS) {
    const last = journal.history[objectId]?.at(-1);
  ...
  try {
    ...
    for (const pass of pending) {
      ...
      const bytes = JSON.stringify({
        scope,
        data: { kind: "final-fence", observation: finalFence },
      });
      if (Buffer.byteLength(bytes) > 262_144) throw new Error("karaoke_cleanup_artifact_limit");
      const id = reconciliationDigest(bytes);
      fresh.set(id, bytes);
    ...
    result = await verifyKaraokeReconciliation(
      {
  ...
  if (result === undefined) throw new Error("karaoke_cleanup_unverified");
