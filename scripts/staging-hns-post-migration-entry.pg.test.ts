...
suite("HNS staging post-migration entry point (PostgreSQL)", () => {
  test("prepares the template with the complete reviewed chain", async () => {
    ...
    await runPostgresMigrations({ connectionString: url, migrations });
    await withAdmin(fixture, async (admin) => {
      const ledger = await admin.query<{ version: string }>(
        "SELECT version FROM api_next.schema_migrations ORDER BY version",
      );
      expect(ledger.rows.at(-1)?.version).toBe(migrations.at(-1)?.version);
    });
  });
  ...
  test("refuses a checksum mismatch and an unpinned migration after the endpoint", async () => {
    ...
    await withAdmin(beyondFixture, async (admin) => {
      await admin.query(
        "INSERT INTO api_next.schema_migrations (version, checksum) VALUES ('9999_unreviewed.sql', repeat('9', 64))",
      );
    });
    ...
    expect(
      ...
      detail: { version: "9999_unreviewed.sql" },
    });
