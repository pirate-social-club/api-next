...
import { DATA_REGISTRATION_MAX_SOURCE_BYTES } from "@pirate/platform-cf/data/registration-artifact-pipeline";
import {
...
describe("DATA registration composition", () => {
  test("admits the video ingest ceiling within the pinning adapter's supported bound", () => {
    expect(DATA_REGISTRATION_MAX_SOURCE_BYTES).toBeGreaterThanOrEqual(
      VIDEO_INGEST_POLICY_V1.maxBytes,
    );
