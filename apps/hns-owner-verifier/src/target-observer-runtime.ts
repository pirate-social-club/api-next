...
import { makeHnsOwnerAuthoritativeDnsTargetObserver } from "./owner-authoritative-source-observer.ts";
...
export async function makeHnsOwnerAuthoritativeTargetObserverRuntime(
  input: Readonly<{
  ...
  if (
    ...
    throw new HnsParentChainObserverError(
      "misconfigured",
      "HNS owner target-observer configuration deadline is invalid",
    );
  }
  if (options.signal.aborted) {
    throw new HnsParentChainObserverError(
      "transport_unavailable",
      "HNS owner target-observer composition was already aborted",
    );
  }
  ...
  try {
    ...
    if (options.signal.aborted) {
      throw new HnsParentChainObserverError(
        "transport_unavailable",
        "HNS owner target-observer configuration resolution was aborted",
      );
    }
    if (error instanceof HnsControlObserverConfigurationError) {
      throw new HnsParentChainObserverError(
        "misconfigured",
        "HNS owner target-observer composition authority is invalid",
      );
    }
    throw new HnsParentChainObserverError(
      "transport_unavailable",
      "HNS owner target-observer configuration registry is unavailable",
    );
  }
  if (options.signal.aborted) {
    throw new HnsParentChainObserverError(
      "transport_unavailable",
      "HNS owner target-observer configuration resolved after abort",
    );
  }
  ...
  if (
    ...
    throw new HnsParentChainObserverError(
      "misconfigured",
      "HNS owner target-observer immutable configuration is incomplete",
    );
  }
  const pinnedConfigurationBytes = new Uint8Array(resolved.configuration_bytes);
  const pinnedCapabilities = Object.freeze({ ...input.capabilities });
  ...
  const target = makeHnsOwnerAuthoritativeDnsTargetObserver({
    configuration_resolver: {
      resolve: async (identity, resolutionOptions) => {
        if (resolutionOptions.signal.aborted) {
          throw new Error("HNS pinned owner target-observer configuration resolution aborted");
        }
        if (
          identity.reference !== runtimeConfiguration.provider_configuration_reference ||
          identity.version !== runtimeConfiguration.provider_configuration_version
  ...
  const observer = Object.freeze({
    observe: async (...parameters: Parameters<typeof target.observe>) => {
      ...
      if (
        ...
        throw new HnsParentChainObserverError(
          "misconfigured",
          "HNS owner observation authority does not match the composed runtime",
        );
      }
