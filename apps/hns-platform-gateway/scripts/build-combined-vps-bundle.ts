const outputPath = "dist/pirate-hns-community-app-handle-gateway.mjs";
...
async function command(arguments_: readonly string[]): Promise<string> {
  ...
  if (exitCode !== 0) throw new Error("Combined gateway build provenance check failed");
  return output.trim();
}

const status = await command(["git", "status", "--porcelain", "--untracked-files=no"]);
if (status !== "") throw new Error("Combined gateway build requires a clean tracked tree");
const sourceCommit = await command(["git", "rev-parse", "HEAD"]);
if (!/^[0-9a-f]{40}$/u.test(sourceCommit)) {
  throw new Error("Combined gateway source commit is invalid");
}

const result = await Bun.build({
  entrypoints: ["src/combined-main.ts"],
  outdir: "dist",
  naming: "pirate-hns-community-app-handle-gateway.mjs",
  target: "bun",
  format: "esm",
  minify: false,
  sourcemap: "none",
...
if (!result.success) throw new Error("Combined gateway bundle build failed");
...
console.log(
  JSON.stringify({
    event: "hns_community_app_handle_gateway_bundle_built",
