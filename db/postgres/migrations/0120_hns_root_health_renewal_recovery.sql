-- Retryable renewal failures retain their job identity and serving fences.
-- attempt_count saturates at 1024; lease_fence remains monotonic. A counter
-- ceiling must never permanently disable an otherwise eligible generation.
ALTER TABLE hns_root_health_renewal_jobs
  ADD COLUMN next_attempt_at TIMESTAMPTZ,
  DROP CONSTRAINT hns_root_health_renewal_jobs_identity_check,
  DROP CONSTRAINT hns_root_health_renewal_jobs_state_check,
  DROP CONSTRAINT hns_root_health_renewal_jobs_state_shape;

CREATE FUNCTION hns_root_health_renewal_terminal_failure_v1(code TEXT)
...
CREATE FUNCTION hns_root_health_renewal_delay_v1(attempt INTEGER)
RETURNS INTERVAL LANGUAGE sql IMMUTABLE AS $$
  SELECT CASE WHEN attempt < 3 THEN 30
    ELSE LEAST(21600, 1800 * power(2, LEAST(GREATEST(attempt - 3, 0), 4))) END
    * interval '1 second'
$$;

UPDATE hns_root_health_renewal_jobs SET
  state = CASE WHEN hns_root_health_renewal_terminal_failure_v1(failure_code)
    THEN 'terminal' ELSE 'delayed' END,
  next_attempt_at = CASE WHEN hns_root_health_renewal_terminal_failure_v1(failure_code)
    THEN NULL ELSE clock_timestamp() + hns_root_health_renewal_delay_v1(attempt_count) END,
  completed_at = CASE WHEN hns_root_health_renewal_terminal_failure_v1(failure_code)
    THEN completed_at ELSE NULL END,
  updated_at = clock_timestamp()
WHERE state = 'failed';

ALTER TABLE hns_root_health_renewal_jobs
  ADD CONSTRAINT hns_root_health_renewal_jobs_identity_check CHECK (
    ...
    AND attempt_count BETWEEN 0 AND 1024
    AND lease_fence BETWEEN 0 AND 9007199254740991
  ),
  ADD CONSTRAINT hns_root_health_renewal_jobs_state_check CHECK (
    state IN ('queued', 'leased', 'delayed', 'completed', 'terminal')
  ),
  ADD CONSTRAINT hns_root_health_renewal_jobs_state_shape CHECK (
    ...
    OR (state = 'leased' AND is_hns_host_persistence_identity(leased_by, 256)
      ...
      AND next_attempt_at IS NULL)
    OR (state = 'delayed' AND leased_by IS NULL AND lease_expires_at IS NULL
      AND result_bytes IS NULL AND result_sha256 IS NULL AND completed_at IS NULL
    ...
    OR (state = 'completed' AND leased_by IS NULL AND lease_expires_at IS NULL
      ...
      AND failure_code IS NULL AND completed_at IS NOT NULL AND next_attempt_at IS NULL)
    OR (state = 'terminal' AND leased_by IS NULL AND lease_expires_at IS NULL
      AND result_bytes IS NULL AND result_sha256 IS NULL
      AND is_hns_host_persistence_identity(failure_code, 128)
      AND completed_at IS NOT NULL AND next_attempt_at IS NULL)
  );

CREATE INDEX hns_root_health_renewal_jobs_delayed_idx
  ON hns_root_health_renewal_jobs(next_attempt_at, renewal_job_id)
  WHERE state = 'delayed';
...
BEGIN
  ...
  FOREACH function_signature IN ARRAY ARRAY[
    'hns_root_health_renewal_terminal_failure_v1(text)',
    'hns_root_health_renewal_delay_v1(integer)',
