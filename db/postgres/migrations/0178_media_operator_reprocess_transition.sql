...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE s.operation_id=NEW.operation_id AND s.submission_id=NEW.submission_id
      ...
      AND NEW.replacement_budget_after=0 AND s.workflow_replacement_sequence=0
      AND e.evidence->'replacement_budget_before'=to_jsonb(NEW.replacement_budget_before)
      AND e.evidence->'replacement_budget_after'=to_jsonb(NEW.replacement_budget_after)
      AND o.payload->'replacement_sequence'='0'::jsonb
...
-- Resetting an already-zero budget does not activate the specialized lyrics/
-- replacement trigger. Its operator audit pair, rather than the ordinary retry
-- event pair, owns this transition in both the zero and nonzero cases.
DO $operator_event_pair$
DECLARE definition TEXT; marker TEXT := E'BEGIN\n'; position INTEGER;
BEGIN
  definition := pg_get_functiondef('validate_media_submission_event_pair()'::regprocedure);
  position := strpos(definition, marker);
  IF position = 0 THEN RAISE EXCEPTION 'media event-pair body was not found'; END IF;
  definition := overlay(definition placing marker || E'  IF OLD.media_kind = ''song'' AND OLD.status = ''processing_failed'' AND OLD.failure_code = ''workflow_terminal_unconverged'' AND NEW.status = ''processing'' AND EXISTS (SELECT 1 FROM media_operator_reprocess_actions a WHERE a.operation_id=NEW.operation_id AND a.submission_id=NEW.submission_id AND a.expected_creation_revision=OLD.creation_revision AND a.expected_workflow_revision=OLD.workflow_revision AND a.resulting_creation_revision=NEW.creation_revision AND a.resulting_workflow_revision=NEW.workflow_revision AND a.replacement_budget_before=OLD.workflow_replacement_sequence AND a.replacement_budget_after=NEW.workflow_replacement_sequence) THEN RETURN NEW; END IF;\n'
    from position for length(marker));
  EXECUTE definition;
END;
$operator_event_pair$;

CREATE FUNCTION guard_media_operator_reset_launch() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.event_type='workflow_replacement' AND NEW.payload->'replacement_sequence'='0'::jsonb
     AND NOT EXISTS (
       SELECT 1 FROM media_operator_reprocess_actions a
       WHERE a.outbox_event_id=NEW.outbox_event_id AND a.operation_id=NEW.operation_id
         AND a.submission_id=NEW.submission_id AND a.community_id=NEW.community_id
         AND a.actor_user_id=NEW.actor_user_id AND a.resulting_creation_revision=NEW.creation_revision
         AND a.resulting_workflow_revision=NEW.workflow_revision AND a.replacement_budget_after=0
     ) THEN RAISE EXCEPTION 'zero replacement sequence requires its operator audit'; END IF;
  RETURN NEW;
END;
$$;
CREATE TRIGGER media_operator_reset_launch_guard BEFORE INSERT ON media_submission_outbox
FOR EACH ROW EXECUTE FUNCTION guard_media_operator_reset_launch();
